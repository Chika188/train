...
export default function BattleResult() {
  ...
  useEffect(() => {
    // 尝试从url参数获取player1/player2
    const searchParams = new URLSearchParams(location.search);
    let player1 = null,
      player2 = null;
    try {
      if (searchParams.get('player1') && searchParams.get('player2')) {
        player1 = JSON.parse(decodeURIComponent(atob(searchParams.get('player1'))));
        player2 = JSON.parse(decodeURIComponent(atob(searchParams.get('player2'))));
      }
    } catch (e) {
      player1 = null;
      player2 = null;
    ...
    if (!player1 || !player2) {
      player1 = JSON.parse(localStorage.getItem('player1Data') || '{}');
      player2 = JSON.parse(localStorage.getItem('player2Data') || '{}');
    }
    // 比较逻辑
  ...
  return (
    <div className='battle-result-container'>
          ...
          {projects.map((project) => (
            <Col key={project.id} xs={24} sm={12}>
              <div key={project.id} className='project-card project-card-battle'>
                <div className='project-rank' style={{ marginBottom: 10 }}>
                  {' '}
                  {project.isWinner ? 'Winner' : 'Loser'}
                </div>
                <img
                ...
                />
                <h4 className='project-name' style={{ marginBottom: 0 }}>
                  Scores: {project.public_repos}
                </h4>
                <h3 className='project-name' style={{ color: '#0396ff' }}>
      ...
      <div className='Battle-reset' onClick={onReset}>
        RESET
      </div>
    </div>
  );
